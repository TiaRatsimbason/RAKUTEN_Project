...
class Predict:
    ...
    def predict(self, df, image_type="test"):
        """
        ...
        try:
            ...
            required_columns = ["description", "imageid", "productid"]
            ...
            logger.info(f"Loading and preprocessing {image_type} images from GridFS...")
            images = []
            ...
            image_paths = self.image_handler.batch_extract_images(df, image_type=image_type)
            ...
            for _, row in df.iterrows():
                image_id = f"{row['imageid']}_{row['productid']}"
                if image_id not in image_paths:
                    raise ValueError(f"Image path not found for {image_id}")
                ...
                try:
                    ...
                    logger.debug(f"Successfully processed image {image_id}")
            ...
            for i, pred in enumerate(final_predictions):
                predictions[str(i)] = str(pred)
                logger.debug(f"Prediction {i}: class {pred}")
...
def main():
    ...
    parser.add_argument(
        "--evaluation",
        action="store_true",
        help="Run in evaluation mode (uses training images)"
    )
    ...
    try:
        ...
        if args.evaluation:
            logger.info("Running in evaluation mode with training data...")
            df = pd.DataFrame(list(sync_db.preprocessed_x_train.find({}, {'_id': 0})))
            image_type = "train"
        else:
            logger.info("Running in prediction mode with test data...")
            df = pd.DataFrame(list(sync_db.preprocessed_x_test.find({}, {'_id': 0})))
            image_type = "test"
            
        logger.info(f"Loaded {len(df)} samples")

        # Faire les prédictions
        predictions = predictor.predict(df, image_type=image_type)
